...
Este programa implementa una pila dinámica para evaluar expresiones postfijas. 
Las operaciones que realiza son de estructuras de datos (pila) y la evaluación de expresiones para 
comprender como se ejecutan las operaciones internamente en lenguaje C. 

Desarrollado por: Diego Manzanero
...
int main()
{
  ...
  *p=inic(p);
  ...
  while(var!='\0')
  {
    if (cad[cont]=='+'||cad[cont]=='-'||cad[cont]=='*'||cad[cont]=='/'||cad[cont]=='^')
    {
      x=isEmpty(p);
      if(x==0)
      {
